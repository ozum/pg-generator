...
function processColumn(schema, column) {
    ...
    let method   = types[column.type];
    let property = schema.properties[column.name] = {};
...
function processInteger(schema, column) {
    let limit    = Math.pow(2, column.precision) / 2;
    let property = schema.properties[column.name];
...
function processNumeric(schema, column) {
    let limit = Math.pow(10, (column.precision - column.scale)) - 1;
    let property = schema.properties[column.name];
