...
function getBelongsToManyName(hasManyThrough) {
    ...
    if (getSpecificConfig(tableName, 'generate.addTableNameToManyToMany')) {
        as = throughTableName + '_' + as;
    }
    if (as.match(/_id$/i)) {
        as = as.replace(/_id$/i, '');                                                       // company_id -> company
    } else {
    ...
    for (ri = 0; ri < templateTable.relations.length; ri = ri + 1) {
        ...
        for (ci = 0, catched = false; ci < templateTable.columns.length && !catched; ci = ci + 1) {
            ...
            if (column.name === relationToValidate.as) {
                console.warn('ERROR: %s is already used. %s relation is trying to have alias of previously defined column with same name in table %s.%s.', relationToValidate.as, relationToValidate.type, templateTable.schema, templateTable.tableName);
        ...
        for (pi = 0, catched = false; pi < ri && !catched; pi = pi + 1) {
            ...
            if (relation.as === relationToValidate.as) {
                console.warn('ERROR: %s is already used. %s relation is trying to have alias of previously defined %s relation in table %s.%s.', relationToValidate.as, relationToValidate.type, relation.type, templateTable.schema, templateTable.tableName);
                duplicates.push(relationToValidate.as);
                catched = true;
            }
        }
...
function generateModelFiles(db, next) {
    ...
    q.drain = function () {
        if (allNamingErrors.length > 0) {
            next(new Error('There are naming errors. Either set "generate.addTableNameToManyToMany" true in configuration or rename your relations.\n' + allNamingErrors.join('\n')));
        } else {
            next(null);
        }
    };
