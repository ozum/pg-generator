...
var addFilters          = require('./util/template-filter').addFilters;
                    ...
                    .on('readable', function() {
                        ...
                        while ((item = this.read())) {
                            ...
                            if (category === 'db') {
                                ...
                                let data = lodash.defaultsDeep({ db: db }, extraDBData, extraData);
                                ...
                                for (let schema of db.schemas.values()) {
                                    ...
                                    let data = lodash.defaultsDeep({ schema: schema }, extraSchemaData, extraData);
                                    let interpolatedTarget = nunjucksFileName.renderString(target, data);
                                    promises.push(pgen._write(template, interpolatedTarget, data));
                                }
                                ...
                                for (let schema of db.schemas.values()) {
                                    for (let table of schema.tables.values()) {
                                        ...
                                        let data = lodash.defaultsDeep({ table: table }, extraTableData, extraData);
                                        let interpolatedTarget = nunjucksFileName.renderString(target, data);
                                        promises.push(pgen._write(template, interpolatedTarget, data));
                                    }
                                }
