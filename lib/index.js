...
const joiSchema = require('./util/joi-schema');
const typeScriptSchema = require('./util/typescript-schema');
const jsDocSchema = require('./util/jsdoc-schema');
...
function getSystemUtils() {
  return {
    ...
    tableJoiSchema:        joiSchema.process,
    tableTypeScriptSchema: typeScriptSchema.process,
    tableJsDocSchema:      jsDocSchema.process,
  };
}
...
function write(pgenInstance, template, target, templateData) {
  ...
  return fs.outputFile(target, content).then(() => {
...
class PGGenerator {
  ...
  writeAll() {
    ...
    return pgStructure(this.connectionParams, this.schemas).then((db) => {
