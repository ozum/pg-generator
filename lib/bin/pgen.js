...
function connectDB(input, answers, callback) {
    ...
    if (dbClient) { dbClient.end(); }
    dbClient = new pg.Client({host: answers.host, port: answers.port, user: answers.user, password: answers.password, database: answers.db || 'template1'});
...
function getSchemas(answers) {
    var done = this.async();
    let exclude = {'information_schema': true, 'pg_catalog': true, 'pg_toast_temp_1': true, 'pg_temp_1': true, 'pg_toast': true};
    ...
    query.on('row', (row) => { if (!exclude[row.schemaName]) { schemas.push(row.schemaName);} });
