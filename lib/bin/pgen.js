...
function getDatabases(answers, callback) {
  ...
  const db = function db() {
    dbClient.query("SELECT datname FROM pg_database WHERE datistemplate = false AND datname <> 'postgres' ORDER BY datname;", (err, result) => {
      if (err) error(err.message);
      result.rows.forEach(row => databases.push(row.datname));
      done(databases);
    });
  };
...
function getSchemas(answers) {
  ...
  dbClient.query('SELECT schema_name AS "schemaName" FROM information_schema.schemata ORDER BY schema_name;', (err, result) => {
    if (err) error(err.message);
